...
def plot_cart_pole_surface(q_values, config, algorithm, save_dir, episode=None):
    ...
    NO INTERPOLATION - shows discrete Q-values as learned.
    """
    ...
    print("   NO INTERPOLATION - showing discrete Q-values")
    ...
    # CREATE DISCRETE GRID (NO INTERPOLATION)
    print("   Creating discrete grid from original data points...")
    
    # Get unique positions for creating discrete grid
    unique_x = np.unique(points[:, 0])
    unique_y = np.unique(points[:, 1])
    
    print(f"   Discrete grid: {len(unique_x)} x {len(unique_y)} points")
    
    # Create meshgrid for discrete points
    X_discrete, Y_discrete = np.meshgrid(unique_x, unique_y)
    Z_discrete = np.zeros_like(X_discrete)
    Z_discrete[:] = np.nan  # Initialize with NaN for missing data
    
    # Fill in the Z values where we have actual data
    for i, (x, y, z) in enumerate(zip(points[:, 0], points[:, 1], values)):
        x_idx = np.where(unique_x == x)[0][0]
        y_idx = np.where(unique_y == y)[0][0]
        Z_discrete[y_idx, x_idx] = z
    
    print(f"   Grid filled with {np.sum(~np.isnan(Z_discrete))} / {Z_discrete.size} data points")
    
    # Create 3D plot
    fig = plt.figure(figsize=(14, 10))
    ax = fig.add_subplot(111, projection='3d')
    
    # Plot discrete surface with visible structure
    surf = ax.plot_surface(X_discrete, Y_discrete, Z_discrete, 
                          cmap=cm.viridis, alpha=0.8,
                          linewidth=0.5, antialiased=False,
                          edgecolor='black')
    
    # Add scatter points to emphasize discrete nature
    ax.scatter(points[:, 0], points[:, 1], values, 
              c=values, cmap=cm.viridis, s=30, alpha=1.0, 
    ...
    # Title
    title = f'3D Surface Plot of Q-Values (DISCRETE)\n{algorithm}'
    if episode is not None:
    ...
    filename = f'{algorithm}_q_surface_Cart_Position_vs_Pole_Angle{episode_str}_discrete.png'
    output_path = os.path.join(save_dir, filename)
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"✅ Saved: {output_path}")
    plt.close()
    
    return points, X_discrete, Y_discrete, Z_discrete


def plot_velocity_surface(q_values, config, algorithm, save_dir, episode=None):
    """
    Create 3D surface: Cart Velocity vs Pole Velocity (averaged over positions).
    NO INTERPOLATION - shows discrete Q-values as learned.
    """
    ...
    # Get unique velocities for creating discrete grid
    unique_x = np.unique(points[:, 0])
    unique_y = np.unique(points[:, 1])
    ...
    # Save
    episode_str = f"_ep{episode}" if episode is not None else ""
    filename = f'{algorithm}_q_surface_Cart_Velocity_vs_Pole_Velocity{episode_str}_discrete.png'
    output_path = os.path.join(save_dir, filename)
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"✅ Saved: {output_path}")
    plt.close()
...
def plot_comparison(all_data, save_dir):
    ...
    plt.suptitle('Q-Value Comparison: All Algorithms (DISCRETE)', fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout()
    
    filename = 'ALL_ALGORITHMS_comparison_discrete.png'
    output_path = os.path.join(save_dir, filename)
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"✅ Saved: {output_path}")
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description="Visualize Q-values: Cart Position vs Pole Angle or Cart Velocity vs Pole Velocity (DISCRETE)")
    ...
    print("\n" + "="*70)
    if args.velocity:
        print("📊 Q-Value Visualization: Cart Velocity vs Pole Velocity (DISCRETE)")
    else:
        print("📊 Q-Value Visualization: Cart Position vs Pole Angle (DISCRETE)")
    print("="*70)
    print(f"Task: {task}")
    ...
    for algorithm in algorithms:
        ...
        if args.velocity:
            points, X, Y, Z = plot_velocity_surface(q_values, config, algorithm, plots_dir, episode)
            all_data[algorithm] = (points, X, Y, Z)
        else:
            points, X, Y, Z = plot_cart_pole_surface(q_values, config, algorithm, plots_dir, episode)
            all_data[algorithm] = (points, X, Y, Z)
